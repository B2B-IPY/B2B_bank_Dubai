...
const TransferirPix: React.FC = () => {
   ...
   const [pixAdressKey, setPixAdressKey] = useState<string>("CHAVE_ALEATORIA");
   const [selectedMask, setSelectedMask] = useState("");
   const [iframeUrl, setIframeUrl] = useState<string | null>(null);

   const handleMaskChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
      const value = e.target.value;
      setPixAdressKey(value);
      setSelectedMask(value);

      switch (value) {
         case "CPF":
            $("#taxId").mask("999.999.999-99");
            break;
         case "CNPJ":
            $("#taxId").mask("99.999.999/9999-99");
            break;
         case "TELEFONE":
            $("#taxId").mask("(99) 99999-9999");
            break;
         default:
            $("#taxId").unmask();
      }
                        ...
                        <div className="w-[90%] flex justify-end py-5 gap-4">
                           <button
                              onClick={() => {
                                 ...
                                 axios
                                    .post(
                                       "https://api.noutbank.com.br/transfers/pix",
                                       {
                                          value: data.amount,
                                          pixAddressKey: data.taxId,
                                          pixAddressKeyType: pixAdressKey,
                                          code: data.code,
                                       },
                                       headers
                                    )
                        ...
                        <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                           ...
                           <form
                              ...
                              onSubmit={(e) => {
                                 ...
                                 // axios
                                 //    .get(
                                 //       `https://api.noutbank.com.br/pix/consultar/${data.taxId}/${pixAdressKey}`,
                                 //       headers
                                 //    )
                                 //    .then((res) => {
                                 //       console.log(res.data);
                                    ...
                                    <div>
                                       ...
                                       <div className="flex w-full gap-5 justify-between">
                                          ...
                                          <select
                                             ...
                                             id="pixAddressKeyType"
                                             className="w-[20%] px-2 bg-transparent border border-gray-300 text-[var(--title-primary-color)] sm:text-sm rounded-lg focus:ring-[var(--primary-color)] focus:border-[var(--primary-color)] block  p-2. max-[1000px]:bg-transparent  max-[1000px]:focus:outline-none max-[1000px]:focus:ring-1 max-[1000px]:focus:border-[var(--primary-color)]"
