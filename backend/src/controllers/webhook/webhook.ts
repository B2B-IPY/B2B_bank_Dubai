...
import { io } from "../../index"; // Importe o io do arquivo index.ts
...
async function webhook(req: Request, res: Response) {
  ...
  if (
    !webhook_respose.uuid ||
    !webhook_respose.bankData ||
    !webhook_respose.transaction ||
    !webhook_respose.status ||
    !webhook_respose.event
  ) {
    return res.status(200).json({ error: "Dados do webhook inválidos" });
  }
  ...
  let tarifa = 0;
  if (object === "PIX_PAY_IN") {
    tarifa = user_info.cashin_fixo + (user_info.cashin_porcent * amount) / 100;
  }
  if (object === "PIX_PAY_OUT") {
    tarifa =
